...
<div id="login">
    ...
    <div class="container" style="width: 550px;height: 400px ;background-color: #5cb85c;margin-top: 30px;border-radius: 6px">
        <form action="${pageContext.request.contextPath}/addmoneyServlet" method="post" class="container offset1"  id="buytickform" style="width: 540px">
            <table style="text-align: center;border: #FFFFFF solid 3px;margin:80px auto;">
                <tr>
                    <td colspan="2"><h3>欢迎进入充值窗口</h3></td>
                </tr>
                <tr>
                    <td ><label>充值账户：</label>
                    <c:if test="${user.power==1}"> <td><input name="userid" placeholder="充值账户"></td></c:if>
                    <c:if test="${user.power==0}"> <td><input name="userid" value="${user.id}" readonly></td></c:if>
                <tr>
                <td ><label>请输入充值金额：</label>
                <td><input name="money" placeholder="充值金额"></td>
                </tr>
                <tr>
            ...
            <c:if test="${addmoney_msg !=null }">  <tr><div class="alert alert-warning alert-dismissible" role="alert" style="margin-bottom: 0px">
                <button type="button" class="close" data-dismiss="alert" >
                    <span>&times;</span>
                </button>
                <strong>${addmoney_msg}</strong>
            </div></tr></c:if>
        </form>
    </div>
</div>
